...
- Cookies actions trio. `cookies_get` reads via `Storage.getCookies` with optional `urls` filter (host match, including parent/subdomain) and `maxResults` (default 200, max 500). `cookies_set` validates that each entry has `url` or `domain` and forwards to `Storage.setCookies`. `cookies_clear` calls `Storage.clearCookies`. All three wrap CDP throws into deterministic fails. Covered by 6 unit tests for filter behavior, maxResults cap, error wrapping, and method routing.
- `wait_for_url` action. Polls `page.currentUrl()` until it matches `pattern` (substring contains-check if no `*`, otherwise wildcard glob with safely-escaped regex metachars). Bounded by `timeoutMs` (default 10s, max 30s). Helper `urlMatchesPattern` + `waitForUrl` live in `page-navigation.ts` and exposed on `Page`. 8 unit tests cover substring/wildcard semantics, anchored matching, regex-metachar escaping, polling, timeout, and swallowed currentUrl throws.
- `set_viewport` action. Wraps `Emulation.setDeviceMetricsOverride` with `width`, `height`, optional `deviceScaleFactor` (default 1), optional `mobile` flag (default false). Returns the applied metrics on `result.data`; CDP throws are caught and surfaced as deterministic fails. 4 unit tests cover defaults, overrides, error wrapping, and data payload.
- Console message capture trio. `console_start` subscribes to `Runtime.consoleAPICalled` + `Runtime.exceptionThrown` and buffers entries (bounded to 1000 entries × 2KB text each). `console_read` returns entries with optional `level` filter (`warn`→`warning` normalized) and `maxResults` (default 100), with `clear=true` to empty the buffer in place. `console_stop` unsubscribes and reports captured count. Each entry exposes `level`, `text` (joined from CDP args), `url`, `lineNumber`, `timestamp`. Uncaught exceptions surface as level `exception`. 8 unit tests cover subscribe shape, idempotent start, arg join, level filter, clear, fail-without-start, exception serialization, and unsubscribe-on-stop.
- `network_list_requests` action. Reads from the active HAR recorder (requires `network_har_start`) and returns filtered request summaries — `urlIncludes` (case-insensitive substring), `method` (case-insensitive), `status` (exact code or `1xx`..`5xx` bucket), `maxResults` (default 50, max 500). Returns `{ total, matched, entries }` where each entry has `requestId`, `method`, `url`, `status`, `mimeType`, and computed `durationMs`. Filter helper `filterNetworkEntries` exported from `network.ts` and covered by 8 unit tests for empty filter, url/method/status filters, missing-response handling, maxResults cap, and duration computation.
